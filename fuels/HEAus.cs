...
{
    internal class HEAus : BaseFuel
    {
        public HEAus() : base("HEAus", 100000000, false, "sqrt(x) * 35 / 10", new FunctionType().SQUARE_ROOT, "x * 0.5", "x² * 50", 0.02, 0.02 * 0.5, 5211)
        {
            return;
        }
        ...
        {
            if (!IsFastFlux) return Math.Sqrt(InFlux) * 35.0 / 10.0;
            return 0;
        }
